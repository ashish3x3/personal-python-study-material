...
connected list
unconneted list = initilzedt to all
tour list
degree map

add to conneted list any x,y edge
increase the degree of x,y = 1
    till uncounted != 0
            now take this x,y edge and try to connect to any of the node from unconneted ndoe               list and add to connted list : 
find odd node and even nodes list based of degree of nodes from degree map
    till odd nodes != 0:
        connect odd nodes together if they are not already connected and add them to even               nodes and add to tour
        If all odd nodes are connected then connect the present odd node to an even list        node and add that even node to odd list and currect node to even list: did not      understood this step
